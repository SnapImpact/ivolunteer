...
public class SourceResource {
    ...
    /**
     * Get method for retrieving an instance of Source identified by id in XML format.
     *
    ...
    public SourceConverter get(@QueryParam("expandLevel")
    @DefaultValue("1")
    int expandLevel) {
    ...
    /**
     * Put method for updating an instance of Source identified by id using XML as the input format.
     *
     * @param id identifier for the entity
    ...
    protected Source getEntity() {
        ...
        try {
            return (Source) em.createQuery("SELECT e FROM Source e where e.id = :id").setParameter("id", id).getSingleResult();
    ...
    protected Source updateEntity(Source entity, Source newEntity) {
        ...
        for (Event value : eventCollection) {
            if (!eventCollectionNew.contains(value)) {
                throw new WebApplicationException(new Throwable("Cannot remove items from eventCollection"));
            }
        }
    ...
    protected void deleteEntity(Source entity) {
        ...
        if (!entity.getEventCollection().isEmpty()) {
            throw new WebApplicationException(new Throwable("Cannot delete entity because eventCollection is not empty."));
        }
